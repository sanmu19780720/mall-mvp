...
### Requirement: 用户端前端工程可正常启动
用户端前端工程 SHALL 基于 Vue 3 + Vite + Vant，执行 `npm run dev` 后可在浏览器访问。
...
用户端 SHALL 提供统一的 Axios 实例，自动注入 JWT token 并处理 401 响应。
...
用户端 SHALL 配置 Vue Router，包含首页和登录页占位路由。
